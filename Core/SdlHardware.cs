 ...
 * 
 * Changes:
 * 0.17  09-mar-2015: Improved full screen speed; added method "log"
...
class SdlHardware
{
    ...
    static DateTime lastDisplayingTime;
    ...
    public static void Init(short w, short h, int colors, bool fullScreen)
    {
        ...
        // 
        lastDisplayingTime = System.DateTime.Now;
    }

    public static void Lock()
    {
        if(Sdl.SDL_MUSTLOCK(hiddenScreen) == 1)
        {
            Sdl.SDL_LockSurface(hiddenScreen);
        }
    }
    ...
    public static void Unlock()
    {
        if(Sdl.SDL_MUSTLOCK(hiddenScreen) == 1)
        {
            Sdl.SDL_UnlockSurface(hiddenScreen);
        }
    }
    ...
    {
        // Instead of waiting exactly "n" milliseconds,
        // let's subtract the time taken in drawing and game logic
        DateTime currentTime = DateTime.Now;
        TimeSpan diff = currentTime - lastDisplayingTime;
        if (diff.Milliseconds > 0)
            milisegundos -= diff.Milliseconds;
        if (milisegundos < 0)
            milisegundos = 0;
        Thread.Sleep(milisegundos);
        lastDisplayingTime = DateTime.Now;
    }
    ...
    {
        StreamWriter sw = File.AppendText("log.log");
        sw.WriteLine(text);
        sw.Close();
    }
    ...
    public static int KEY_RM = Sdl.SDLK_BACKSPACE;
